...
import '../api/paged-items.dart';

typedef FetchFunction<T> = Future<PagedItems<T>> Function(int startingIndex, int itemsPerPage);
class PageItemsNotifier<T> extends ChangeNotifier {
  final FetchFunction<T> fetchPage;
  final int numberOfItemsPerPage;
  final T defaultItem;
  PageItemsNotifier({
    required this.fetchPage,
    required this.numberOfItemsPerPage,
  ...
  /// access.
  final Map<int, PagedItems<T>> _pages = {};
  ...
  T getByIndex(int index) {
    ...
    var startingIndex = (index ~/ numberOfItemsPerPage) * numberOfItemsPerPage;
  ...
  Future<void> _fetchPage(int startingIndex) async {
    ...
    final page = await fetchPage(startingIndex, numberOfItemsPerPage);
